...
func connectZitiService(ctxCollection *ziti.CtxCollection, opts map[string]string) {
	...
	log.Printf("Calling PetstoreDemo with url: %s", "http://PetstoreDemo"+opts["query"])
	// Get a zitified httpClient that can dial (call) a Ziti service by name
	httpClient := buildZitiClient(ctxCollection)
	// Use the httpClient as normal, using the service name as the address
