...
psql -v --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
	...
	ALTER DATABASE simpledb OWNER TO $POSTGRES_USER;

  CREATE ROLE $POSTGRES_VIEW_USER WITH LOGIN PASSWORD '$POSTGRES_VIEW_PASSWORD';
  GRANT CONNECT ON DATABASE simpledb TO $POSTGRES_VIEW_USER;

	\connect simpledb;

  GRANT USAGE ON SCHEMA public TO $POSTGRES_VIEW_USER;
  GRANT SELECT ON ALL TABLES IN SCHEMA public TO $POSTGRES_VIEW_USER;
  GRANT SELECT ON ALL SEQUENCES IN SCHEMA public TO $POSTGRES_VIEW_USER;
  ALTER DEFAULT PRIVILEGES IN SCHEMA public
  GRANT SELECT ON TABLES TO $POSTGRES_VIEW_USER;
  ALTER DEFAULT PRIVILEGES IN SCHEMA public
  GRANT SELECT ON SEQUENCES TO $POSTGRES_VIEW_USER;
